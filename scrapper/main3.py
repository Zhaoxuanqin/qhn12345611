...
def get_all_tweets_count(runConfig, scrapFrom='twitter', demoMode=False):
    api, myfirebase = setup(runConfig)
    runMode = runConfig[scrapFrom]["runMode"]
    fromAccTmp = []
    for a in runConfig[scrapFrom][runMode]['account']:
        a=a[1:]
    ...
    startDay = runConfig[scrapFrom]['start']
    endDay = runConfig[scrapFrom]['end']
    result = api.get_all_tweets_count(query=f'{fromAcc} {query}',start_time = startDay,end_time= endDay).meta['total_tweet_count']
    print(f'{fromAcc} {query}: {result} times')
    return result
