...
export const getDate=(eleId,exclusive=false)=>{
    ...
    if(eleId=='getfromDate'){
        ...
        if(exclusive){
            temp =new Date(temp.getFullYear(),temp.getMonth(),temp.getDate()-1,23,59,59)
        }
        else{
            temp =new Date(temp.getFullYear(),temp.getMonth(),temp.getDate(),23,59,59)
        }
    }
    return temp
}
...
export const maskName = (split_word : string, text : string) =>{

    let mask = ""
    for(let i =0;i<split_word.length;i++){
        mask = "✱"+mask
    }
    var str = text.split(split_word)
    var previous = str[0]
    var later = str[1]
    ...
    if(typeof(later)=="undefined"){
        for( let i =0;i <previous.length;i++){
            previous_result.push(previous[i].toString())
        }
        return previous_result.join("")+mask
    }
    else{
        for(let i =0;i<previous.length;i++){
            previous_result.push(previous[i].toString())
        ...
        for(let i =0;i<later.length;i++){
            later_result.push(later[i].toString())
        }
    }
    return previous_result.join("")+mask+later_result.join("")
}
...
export const maskAddress = (split_word : string, text : string) =>{
    var str = text.split(split_word)
    var previous = str[0]
    var later = str[1]
    ...
    if(typeof(later)=="undefined" || later.length<4){
        for( let i =0;i <previous.length -4 ;i++){
            previous_result.push(previous[i].toString())
        }
        return previous_result.join("")+'✱✱✱✱✱✱✱✱✱'
    }
    else{
        for(let i =0;i<previous.length-4;i++){
            previous_result.push(previous[i].toString())
        ...
        for(let i =4;i<later.length;i++){
            later_result.push(later[i].toString())
        }
    }
    return previous_result.join("")+'✱✱✱✱✱✱✱✱✱'+later_result.join("")
}

export const maskPersonalDetails_AtSign = (text:string) => {
    
 
    var name_filter = require('people-names');
    let name_conditions = name_filter.parseNames(text)
    for(let i =0;i<name_conditions.length;i++){
        text = maskName(name_conditions[i],text)
    }
    let conditions= ["Road","Street","Avenue","Lane","STREET","AVENUE","ROAD","LANE","TERRACE" ]
    let check_conditions = []
    
    //check if the text include those condition string, and return the strings 
    for(let i =0;i<conditions.length;i++){
        if(text.includes(conditions[i])){
            check_conditions.push(conditions[i])
        }
 
 
    }
    
    //use funtion to process text
    for(let i =0;i<check_conditions.length;i++){
        text = maskAddress(check_conditions[i],text)
    }
...
export const fetchData = async (accs: string[], kws:string[][], limit= 50
    ...
    let res = null
    accs = accs.map(accs => accs.toLowerCase());
    if (kws.length == 0){
    ...
    if (fromDate == null){
        ...
        if ((refillCounter == 0) && (isOldData || !isReachLimit)) {
            ...
            await refillData(accs,kws, isChecked('isPremium'), null, null)
        }
    } else {
        if (refillCounter==0 && !isReachLimit){
            toast.info('Auto scrapping tweets', { autoClose: 20000 })
            await refillData(accs,kws, isChecked('isPremium'), fromDate, toDate)
        }
            
    }
    console.log(`fetched ${res.length} tweets`)
    if(res.length == 0){
        toast.info("No data has been found, please search again")
    }
...
export const getCountRecent = async (acc: string [],kws: string[],fromDate,toDate) => {
    // let tmp = await Axios.post(`http://${host}:8001/trigger/account`, { list: acc })
    ...
    console.log(`${host_scrapper}${port}/count/all`)
    let data = {list: acc,  keyword: kws, outsideTagIsAND: true, begin: fromDate, stop:toDate}
    let tmp = await Axios.post(`${host_scrapper}${port}/count/all`,  data)
    return tmp
}
...
export const processPredict_fromTotalFetch = async (accInputId, kwsInputId, numberPredict, totalFetch, sortBy,fromDate=null, toDate=new Date()) => {
