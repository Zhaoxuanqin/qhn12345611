...
// const PostView2 = (props) => {
    ...
    postAfter= new Date();
    // dateUntil = 
    default2Date = new Date(this.postAfter.setDate(this.postAfter.getDate())).toISOString().split("T")[0]
    defaultfromDate = new Date(this.postAfter.setDate(this.postAfter.getDate()-28)).toISOString().split("T")[0]
    state = {
        name: 'PostView2',
    ...
    getCount = async () => {
        let tmp = 0
        await getCountRecent(getTagsInput('searchAcc', true, false, !this.state.loading)
                    ,getKwInput('searchKey', false, !this.state.loading)
                    ,getDate('getfromDate')
                    ,getDate('gettoDate')).then(data => {
            tmp = data.data
        }).catch(e => {})
        this.setState({
                tweetCount: tmp
    ...
    fetchMoreData = async () => {
        ...
        if(sortBy !== 'like' && sortBy !='retweet'){
            sortBy = this.sortBy =='date' ? 'postAt' : 'engage'
        }
    ...
    searchDataInfo=(fromDate,toDate, check) =>{
        let from =getDate(fromDate)
        let until =getDate(toDate)
        let result =''
        if( check && from){
            result = `appears on Twitter ${check} times from ${ this.date2string(from) } to ${this.date2string(until)}`
        }
        else if(check){
            result =`appears on Twitter ${check} times before ${this.date2string(until)}`
        }
        return result 
    }
                ...
                {(findAccess(this.state.auth)) ? (<Container position="relative" maxW="12xl">
                    ...
                    <Flex my={2} align="center" justify="center">
                        ...
                        <div id="isPremium">
                            ...
                            <Checkbox colorScheme='blue' defaultChecked={true} isDisabled={true} mx={2}
                                      color={'twitter.600'}>
                                <b>PREMIUM SEARCH</b>
                            </Checkbox>
                        ...
                        <Flex  my={2} align="center" justify="center" >
                                <form>
                                    <label>From: </label>
                                    <input type="date" id='getfromDate' defaultValue={this.defaultfromDate} max={this.default2Date}></input>
                                    <label>Until:</label>
                                    <input type="date" id='gettoDate' defaultValue={this.default2Date} max={this.default2Date}></input>
                                </form>
                    ...
                    <Flex my={2} align="center" justify="center">
                        ...
                        {(this.receivedData && this.receivedData.length > 0) ? this.searchDataInfo('getfromDate','gettoDate',this.state.tweetCount) : ''}
                    </Flex>
